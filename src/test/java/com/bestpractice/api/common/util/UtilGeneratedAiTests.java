...
public class UtilGeneratedAiTests {
    ...
    @Test
    void testGetSpringProfileActiveHandlesNull() {
        // GIVEN
        try (MockedStatic<System> mockedSystem = Mockito.mockStatic(System.class)) {
    ...
    @Test
    void testGetSpringProfileActiveHandlesException() {
        // GIVEN
        try (MockedStatic<System> mockedSystem = Mockito.mockStatic(System.class)) {
            mockedSystem.when(() -> System.getenv("SPRING_PROFILES_ACTIVE")).thenThrow(new RuntimeException("Environment variable access error"));
