...
public class RdbmsInfoPersistentRepositoryGeneratedAiTests {
  ...
  void insert() {
    ...
    assertEquals("test-id", insertedInfo.getId());
  ...
  void replace() {
    ...
    // WHEN
    Info replacedInfo = repository.replace(id, new Info());
    // THEN
    assertEquals(newTitle, replacedInfo.getTitle());
    assertEquals(newDescription, replacedInfo.getDescription());
  }
