...
class LocalUserPersistentRepositoryGeneratedAiTests {
    ...
    void findByEmail_returnsUserIfFound() {
        // GIVEN: A user with a specific email is added to the repository.
        LocalUserPersistentRepository repository = new LocalUserPersistentRepository();
        User user = new User("1", "test", "test@example.com", "password");
        repository.users.add(user);

        // WHEN: The findByEmail("test@example.com") method is called.
        // THEN: The user object with the specified email is returned.
        User returnedUser = repository.findByEmail("test@example.com");
        assert returnedUser != null;
        assert returnedUser.getEmail().equals("test@example.com");
    }
    ...
    void findByEmail_returnsNullIfUserNotFound() {
        ...
        User returnedUser = repository.findByEmail("nonexistent@example.com");
        assert returnedUser == null;
    }

    @Test
    void findById_returnsUserIfFound() {
        // GIVEN: A user with a specific ID is added to the repository.
        LocalUserPersistentRepository repository = new LocalUserPersistentRepository();
        User user = new User("1", "test", "test@example.com", "password");
        repository.users.add(user);
    ...
    void findById_returnsNullIfUserNotFound() {
        ...
        // THEN: Null is returned.
        User returnedUser = repository.findById("nonexistent_id");
        assert returnedUser == null;
    }
    ...
    void insert_returnsInsertedUser() {
        ...
        assert returnedUser != null;
        assert returnedUser.getId().equals("1");
        assert repository.users.contains(returnedUser);
    }
}
