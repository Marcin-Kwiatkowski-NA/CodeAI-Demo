...
class MongoInfoPersistentRepositoryGeneratedAiTests {
    ...
    void testInsert_ShouldInsertAndReturnInfo() {
        ...
        Info info = new Info();
        MongoInfoEntity entity = mock(MongoInfoEntity.class);
        try (MockedStatic<MongoInfoEntity> mocked = Mockito.mockStatic(MongoInfoEntity.class)) {
            mocked.when(() -> MongoInfoEntity.convertFrom(info)).thenReturn(entity);
    ...
    void testReplace_ShouldReplaceAndReturnInfo() {
        ...
        try (MockedStatic<MongoInfoEntity> mocked = Mockito.mockStatic(MongoInfoEntity.class)) {
            ...
            // THEN
            assertThat(result).isEqualTo(info);
            verify(collection).replaceOne(any(Bson.class), eq(entity), any());
        }
    }
    ...
    void testReplace_ShouldThrowInternalServerErrorOnException() {
        ...
        try (MockedStatic<MongoInfoEntity> mocked = Mockito.mockStatic(MongoInfoEntity.class)) {
            mocked.when(() -> MongoInfoEntity.convertFrom(info)).thenReturn(entity);
            when(collection.replaceOne(any(Bson.class), any(), any())).thenThrow(new RuntimeException("Replace failed"));
